...
// 队列已满
$de_queue = new \DEQue\Queue('double_queue', \DEQue\Type::UNRESTRICTED, 2);
...
// 头部入队限制
$de_queue = new \DEQue\Queue('double_queue', \DEQue\Type::FRONT_ONLY_OUT, 10);
$resp = $de_queue->pushFront(new \DEQue\Item('a'));
assert($resp->error()==\DEQue\ErrCode::FRONT_ENQUEUE_RESTRICTED);

// 头部出队限制
$de_queue = new \DEQue\Queue('double_queue', \DEQue\Type::FRONT_ONLY_IN, 10);
$de_queue->pushFront(new \DEQue\Item('a'));
$resp = $de_queue->popFront();
...
// 尾部入队限制
$de_queue = new \DEQue\Queue('double_queue', \DEQue\Type::REAR_ONLY_OUT, 10);
$resp = $de_queue->pushRear(new \DEQue\Item('a'));
assert($resp->error()==\DEQue\ErrCode::REAR_ENQUEUE_RESTRICTED);

// 尾部出队限制
$de_queue = new \DEQue\Queue('double_queue', \DEQue\Type::REAR_ONLY_IN, 10);
...
// 入队与出队方向一致限制
$de_queue = new \DEQue\Queue('double_queue', \DEQue\Type::SAME_IN_OUT, 10);
$de_queue->pushFront(new \DEQue\Item('a'));
$resp = $de_queue->popRear();
