# php-request-id-generator

PHP 唯一请求 ID 生成类

## 介绍

PHP 唯一请求 ID 生成类，使用 `session_create_id()` 与 `uniqid()` 方法，保证唯一性。
...
```php
$generator = \RequestIdGenerator\Factory::make(\RequestIdGenerator\Type::RANDOM);
